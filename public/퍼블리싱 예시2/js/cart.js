...
function removeCart() {
  let deleteCookie = document.getElementsByClassName("deleteBtn");
  for(let i = 0; i < deleteCookie.length; i++) {
    deleteCookie[i].addEventListener("click", () => {
      deleteCookie[i].parentNode.parentNode.remove();
    });
  }
}